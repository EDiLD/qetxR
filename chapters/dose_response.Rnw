...
The following code creates figure \ref{fig:drc_res}:\todo{caption}
...
\subsection{Control mortality in dose-response models}
...
\sidenote{This can be attributed to the additional parameter that needs to be estimated from the data.}
