...
\begin{marginfigure}
<<pyrifos_gammarus, echo=FALSE, fig.width=7, fig.height=3.5>>=
...
\begin{margintable}
  \caption{3 ditches observed for 4 weeks and a possible permutation.}
  \begin{tabular}{ccc}
  Week & Ditch & Perm \\ 
  1 & 1 & 3 \\ 
  2 & 1 & 3 \\ 
...
barplot(pyrifos_prc$CCA$eig[1:6]/sum(pyrifos_prc$CCA$eig)*100, ylab='Explained variance [%]', las = 2, 
